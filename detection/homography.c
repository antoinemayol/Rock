...
int **homography_transform(double **A, int h, int w, int* corners)
{
    ...
    for(int i = 0; i < 8; i++)
    {
        if(i%2 == 0)
        {
            ...
            X[i][4] = 0;
            X[i][5] = 0;
            X[i][6] = -bis[2*i]*corners[2*i];
            X[i][7] = bis[2*i]*corners[2*i+1];
        }
        else
        {
            X[i][0] = 0;
            ...
            X[i][4] = corners[2*i+1];
            X[i][5] = 1;
            X[i][6] = -bis[2*i+1]*corners[2*i];
            X[i][7] = bis[2*i+1]*corners[2*i+1];
        }
    }
    ...
    inverse(inv,n);
    double **M3 = dot_matrix(inv,transpose, n, n, n, n);
...
{
    int *src = 
    int **arr = homography_transform(src, w, h,)
    for(int i = 0; i < size; i++)
    {
        for(int j = 0; j < size; j++)
        {
